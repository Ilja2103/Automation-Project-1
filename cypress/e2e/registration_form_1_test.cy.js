...
describe('This is first test suite Ilja Ollõkainen', () => {
    it('User can submit data only when valid mandatory values are added', () => {
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('input[name="password"]').type('Password2103')
        cy.get('[name="confirm"]').type('Password2103')
        cy.get('#username').type('SomethingSaun')
    ...
    */
    ...
    it('User cannot submit data when phone number is absent', () => {
        ...
        cy.get('input[name="password"]').type('Password2103')
        cy.get('[name="confirm"]').type('Password2103')
        cy.get('[data-testid="phoneNumberTestId"]').type('empty') 
    ...
    it('User cannot submit data when password and/or confirmation password is absent', () => {
        ...
        cy.get('#username').type('SaunMõdu')
        cy.get('[data-testid="phoneNumberTestId"]').type('10203040')
        cy.get('input[name="password"]').type('pw1234')
        cy.get('[name="confirm"]').type('pw1234')
        cy.get('.submit_button').should('not.be.enabled')
        cy.get('#success_message').should('not.be.visible')
    ...
    it('User cannot add letters to phone number', () => {
        ...
        cy.get('input[name="password"]').type('Password2103')
        cy.get('[name="confirm"]').type('Password2103')
        cy.get('[data-testid="phoneNumberTestId"]').type('2')

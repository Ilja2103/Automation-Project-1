...
describe('Section 1: Functional tests', () => {
    ...
    it('User can use only same both first and validation passwords', () => {
        ...
        cy.get('#email').type('saun@gmail.com')
        cy.get('[data-cy="name"]').type('Saun')
        cy.get('#lastName').type('Mõdu')
        cy.get('[data-testid="phoneNumberTestId"]').type('112')
        // Type confirmation password which is different from first password
        cy.get('input[name="password"]').type('PW')
        cy.get('[name="confirm"]').type('PW123')
...
function inputValidData(username) {
    ...
    cy.get('#lastName').type('Mõdu')
    cy.get('[data-testid="phoneNumberTestId"]').type('112')
    cy.get('#password').type('PW123')
    cy.get('#confirm').type('PW123')
    cy.get('h2').contains('Password').click()
}